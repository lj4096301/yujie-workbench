...
| 字号（文本档） | 12（辅助）/ 14（正文）/ 16 / 18（标题）/ 20 / 24 |
| 字号（展示档） | `--fs-display-sm` 28 / `--fs-display-md` 36 / `--fs-display-lg` 48 |
...
| Token | 值 | 用途 |
|---|---|---|
| `--fs-display-sm` | 28px | 次级大数字（温度次档、时钟）、大号入口图标 |
| `--fs-display-md` | 36px | 主数字（温度）、空态图标 |
| `--fs-display-lg` | 48px | 首屏主数字（首页天气卡大温度） |

**规则**：

- 展示档**只给数字与 emoji 图标**用，正文与标题一律不得使用。
- 同一元素在窄屏（`<600px`）允许降一档（48 → 36），但**只能从这三档里取**，禁止自造 22 / 30 / 34 / 40 / 44 / 52 等中间值。
- 展示档数字同样强制 `tabular-nums` / `--font-num`。
...
- 间距**只能取 4 / 8 / 12 / 16 / 24**，禁止 2 / 3 / 5 / 6 / 10 这类中间值。
- 文本元素**必须显式声明 `line-height`**，不得只依赖 body 继承（12px 辅助文字与 14px 正文均为 **1.5**）。
- 列表条目间距不小于 8px；带光环的状态圆点（5px 点 + 光环）视觉尺寸约 13px，条目间距需 ≥8px 才不与相邻行粘连。
...
### 2.8 卡片分区规范（标题栏 + 内容区 + 功能分区）

首页与模块页的卡片一律按「**标题栏 / 内容区**」两段式组织，内容区再按**功能**分块展示。

**标题栏**（`.arco-card-header`）：

| 项 | 值 |
|---|---|
| 高度 | **40px** |
| 背景 | `--bg-subtle` `#F7F8FA`（与白色内容区形成明确区分） |
...
| 项 | 值 |
|---|---|
| 分区小标题 | **12px / 500 / `--text-secondary`**，与内容留 8px |
| 相邻分区 | 上间距 12px + `1px solid var(--border-light)` 分隔线（首分区不加） |

**规则**：

- 分区按**功能语义**命名，禁止「信息 / 详情」这类空词：概览 / 状态分布 / 最近条目 / 当前天气 / 逐小时。
- 单一功能区**建议不画小标题**；但内容本身是复合部件（如 tactile 天气部件内含当前 + 预报）时，应给出小标题说明这块是什么。
